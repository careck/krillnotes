...
[ ] remove the save dialog from "new" and open file dialog from "open". Instead add a settings dialog and data structure where the user can set the default workspace directory. All new workspaces will then automatically be created in this directory. Loading a workspace can have a new dialog which lists all available workspaces in that directory and allows for loading of any existing workspace (which is not currently open). There should be default workspace directory which makes sense depending on the operating system (e.g. ~/Documents/Krillnotes). The settings should  be stored in a location which makes sense for each OS (e.g. ~/.krillnotes/settings.json). Remember that this will also change the way Import works, as it can now create the new workspace directly in the default directory instead of having to ask for a path.
