...
fn load_contacts_example(ws: &mut Workspace) {
    ...
    ws.create_user_script(view_src)
        .expect("contacts view script should load");
    ws.create_user_script(schema_src)
        .expect("contacts schema script should load");
}
...
fn test_is_expanded_defaults_to_true() {
    ...
    assert!(
        child.is_expanded,
        "New child note should be expanded by default"
    );
}
...
fn test_set_and_get_selected_note() {
    ...
    assert_eq!(
        selected,
        Some(root.id.clone()),
        "Should return selected note ID"
    );
...
fn test_selected_note_persists_across_open() {
    ...
    assert_eq!(
        selected,
        Some(root.id),
        "Selection should persist across open"
    );
}
...
fn test_set_selected_note_overwrites_previous() {
    ...
    assert_eq!(
        selected,
        Some(child_id.clone()),
        "Should overwrite previous selection"
    );
}
...
fn test_create_note_root() {
    ...
    ws.storage
        .connection_mut()
        .execute("DELETE FROM notes WHERE id = ?", [&existing_root.id])
        .unwrap();
...
fn test_sibling_insertion_does_not_create_duplicate_positions() {
    ...
    assert_ne!(
        ...
        "child1 and child2 should not share a position"
    );
    assert_ne!(
        child2.position, child3.position,
        "child2 and child3 should not share a position"
    );
    assert_ne!(
        child1.position, child3.position,
        "child1 and child3 should not share a position"
    );
}
...
fn test_get_note_with_corrupt_fields_json_returns_error() {
    ...
    assert!(
        result.is_err(),
        "get_note should return Err for corrupt fields_json"
    );
}
...
fn test_list_all_notes_with_corrupt_fields_json_returns_error() {
    ...
    assert!(
        result.is_err(),
        "list_all_notes should return Err for corrupt fields_json"
    );
}
...
fn test_sibling_insertion_preserves_correct_order() {
    ...
    assert_eq!(
        child3.position, 1.0,
        "child3 (inserted after child1) should be at position 1"
    );
    assert_eq!(
        child2.position, 2.0,
        "child2 should be bumped to position 2"
    );
}
...
fn test_update_note() {
    ...
    new_fields.insert(
        "body".to_string(),
        FieldValue::Text("Updated body".to_string()),
    );

    let updated = ws
        .update_note(&note_id, new_title.clone(), new_fields.clone())
        .unwrap();
    ...
    assert_eq!(
        ...
        Some(&FieldValue::Text("Updated body".to_string()))
    );
...
fn test_delete_note_recursive() {
    ...
    let child2_id = ws
        .create_note(&root_id, AddPosition::AsChild, "TextNote")
        .unwrap();
    let grandchild1_id = ws
        .create_note(&child1_id, AddPosition::AsChild, "TextNote")
        .unwrap();
...
fn test_delete_note_promote() {
    ...
    let middle_id = ws
        ...
        .unwrap();
    let child1_id = ws
        .create_note(&middle_id, AddPosition::AsChild, "TextNote")
        .unwrap();
    let child2_id = ws
        .create_note(&middle_id, AddPosition::AsChild, "TextNote")
        .unwrap();
...
fn test_delete_note_promote_no_position_collision() {
    ...
    let sib1_id = ws
        ...
        .unwrap();
    let sib2_id = ws
        .create_note(&sib1_id, AddPosition::AsSibling, "TextNote")
        .unwrap();
    let child1_id = ws
        .create_note(&sib1_id, AddPosition::AsChild, "TextNote")
        .unwrap();
    let child2_id = ws
        .create_note(&child1_id, AddPosition::AsSibling, "TextNote")
        .unwrap();
    ...
    // sib1 must be gone
    assert!(
        notes.iter().all(|n| n.id != sib1_id),
        "sib1 should be deleted"
    );

    // Gather positions of the surviving root-level notes
    let root_level: Vec<_> = notes
        .iter()
        .filter(|n| n.parent_id == Some(root.id.clone()))
        .collect();
    ...
    assert_eq!(
        positions.len(),
        unique_count,
        "Positions after promote must be unique, got: {:?}",
        positions
    );
    ...
    assert!(
        surviving_ids.contains(&sib2_id),
        "sib2 should remain at root level"
    );
    assert!(
        surviving_ids.contains(&child1_id),
        "child1 should be promoted to root level"
    );
    assert!(
        surviving_ids.contains(&child2_id),
        "child2 should be promoted to root level"
    );
}
...
fn test_update_contact_derives_title_from_hook() {
    ...
    fields.insert(
        "last_name".to_string(),
        FieldValue::Text("Walker".to_string()),
    );
    fields.insert("phone".to_string(), FieldValue::Text("".to_string()));
    fields.insert("mobile".to_string(), FieldValue::Text("".to_string()));
    fields.insert("email".to_string(), FieldValue::Email("".to_string()));
...
fn test_workspace_created_with_starter_scripts() {
    ...
    assert!(
        !scripts.is_empty(),
        "New workspace should have starter scripts"
    );
...
fn test_update_user_script() {
    ...
    let (updated, errors) = workspace
        .update_user_script(&script.id, new_source)
        .unwrap();
    assert!(errors.is_empty());
...
fn test_delete_user_script() {
    ...
    let (script, _) = workspace.create_user_script(source).unwrap();
    assert_eq!(
        workspace.list_user_scripts().unwrap().len(),
        initial_count + 1
    );
...
fn test_user_scripts_sorted_by_load_order() {
    ...
    let scripts = workspace.list_user_scripts().unwrap();
    assert_eq!(
        scripts[0].name, "First",
        "Reordered script should come first"
    );
...
fn test_delete_note_with_strategy() {
    ...
    let result = ws
        .delete_note(&child_id, DeleteStrategy::DeleteAll)
        .unwrap();
    assert_eq!(result.deleted_count, 1);
    ...
    let grandchild_id = ws
        .create_note(&child2_id, AddPosition::AsChild, "TextNote")
        .unwrap();

    let result = ws
        .delete_note(&child2_id, DeleteStrategy::PromoteChildren)
        .unwrap();
    assert_eq!(result.deleted_count, 1);
...
fn test_move_note_logs_operation() {
    ...
    assert_eq!(
        move_ops.len(),
        1,
        "Expected one MoveNote operation in always-on log"
    );
}
...
fn test_create_user_script_rejects_compile_error() {
    ...
    assert_eq!(
        scripts.len(),
        initial_count,
        "No script should be saved on compile error"
    );
}
...
fn test_update_user_script_rejects_compile_error() {
    ...
    assert!(
        result.is_err(),
        "Should return error for invalid Rhai on update"
    );
    ...
    assert_eq!(
        scripts.len(),
        initial_count + 1,
        "Script count must be unchanged after failed update"
    );
...
fn test_on_add_child_hook_fires_on_create() {
    ...
    let root = ws.list_all_notes().unwrap()[0].clone();
    let folder_id = ws
        .create_note(&root.id, AddPosition::AsChild, "Folder")
        .unwrap();

    // Create an Item under the Folder — this should trigger the hook
    ws.create_note(&folder_id, AddPosition::AsChild, "Item")
        .unwrap();
...
fn test_on_add_child_hook_fires_for_sibling_under_hooked_parent() {
    ...
    let first_item_id = ws
        .create_note(&folder_id, AddPosition::AsChild, "Item")
        .unwrap();
    // Second item created as sibling of first (still a child of Folder, hook should fire again, count=2)
    ws.create_note(&first_item_id, AddPosition::AsSibling, "Item")
        .unwrap();
...
fn test_on_add_child_hook_does_not_fire_for_root_level_creation() {
    ...
    assert!(
        result.is_ok(),
        "sibling of root should succeed without hook"
    );
}
...
fn test_on_add_child_hook_fires_on_move() {
    ...
    let folder_id = ws
        ...
        .unwrap();
    let item_id = ws
        .create_note(&root.id, AddPosition::AsChild, "Item")
        .unwrap();
...
fn test_run_tree_action_reorders_children() {
    ...
    let child_b_id = ws
        ...
        .unwrap();
    ws.update_note_title(&child_b_id, "B Note".to_string())
        .unwrap();

    // Create second child as sibling: "A Note" (position 1)
    let child_a_id = ws
        .create_note(&child_b_id, AddPosition::AsSibling, "TextNote")
        .unwrap();
    ws.update_note_title(&child_a_id, "A Note".to_string())
        .unwrap();
...
register_menu("Add Item", ["TaFolder"], |folder| {
    ...
    let root = ws.list_all_notes().unwrap()[0].clone();
    let folder_id = ws
        .create_note(&root.id, AddPosition::AsChild, "TaFolder")
        .unwrap();
...
register_menu("Mark Done", ["TaTask"], |note| {
    ...
    let root = ws.list_all_notes().unwrap()[0].clone();
    let task_id = ws
        .create_note(&root.id, AddPosition::AsChild, "TaTask")
        .unwrap();
...
register_menu("Add Sprint With Task", ["TaSprint"], |sprint| {
    ...
    let root = ws.list_all_notes().unwrap()[0].clone();
    let sprint_id = ws
        .create_note(&root.id, AddPosition::AsChild, "TaSprint")
        .unwrap();

    ws.run_tree_action(&sprint_id, "Add Sprint With Task")
        .unwrap();
    ...
    assert_eq!(
        task_children.len(),
        1,
        "one task expected under child sprint"
    );
...
register_menu("Create Then Fail", ["TaErrFolder"], |folder| {
    ...
    let root = ws.list_all_notes().unwrap()[0].clone();
    let folder_id = ws
        .create_note(&root.id, AddPosition::AsChild, "TaErrFolder")
        .unwrap();
...
fn test_note_tags_round_trip() {
    ...
    ws.update_note_tags(&root.id, vec!["rust".into(), "design".into()])
        .unwrap();
    let note = ws.get_note(&root.id).unwrap();
...
fn test_get_all_tags_sorted_distinct() {
    ...
    ws.update_note_tags(&root.id, vec!["rust".into(), "design".into()])
        .unwrap();
    ws.update_note_tags(&child_id, vec!["rust".into(), "testing".into()])
        .unwrap();
...
fn test_get_notes_for_tag() {
    ...
    ws.update_note_tags(&child_id, vec!["design".into()])
        .unwrap();
    ...
    let both = ws
        .get_notes_for_tag(&["rust".into(), "design".into()])
        .unwrap();
...
fn test_sync_note_links_inserts_row() {
    ...
    ws.update_note(&source.id, source.title.clone(), fields)
        .unwrap();
...
fn test_sync_note_links_removes_row_when_cleared() {
    ...
    ws.update_note(&source.id, source.title.clone(), fields2)
        .unwrap();
...
fn test_delete_note_nulls_links_in_other_notes() {
    ...
    ws.delete_note(&target.id, DeleteStrategy::DeleteAll)
        .unwrap();
...
fn test_delete_note_recursive_clears_links_for_entire_subtree() {
    ...
    let child_id = ws
        .create_note(&parent.id, AddPosition::AsChild, "LinkTestType")
        .unwrap();
    let child = ws.get_note(&child_id).unwrap();
    ...
    ws.update_note(&observer.id, observer.title.clone(), fields)
        .unwrap();

    ws.delete_note(&parent.id, DeleteStrategy::DeleteAll)
        .unwrap();
...
fn test_get_notes_with_link_returns_linking_notes() {
    ...
    for source in [&source1, &source2] {
        let mut fields = BTreeMap::new();
        fields.insert("link".into(), FieldValue::NoteLink(Some(target.id.clone())));
        ws.update_note(&source.id, source.title.clone(), fields.clone())
            .unwrap();
    }
...
fn test_search_notes_matches_title() {
    let mut ws =
        create_test_workspace_with_schema(r#"schema("LinkTestType", #{ version: 1, fields: [] })"#);
    let note = create_note_with_type(&mut ws, "LinkTestType");
    ws.update_note(&note.id, "Fix login bug".into(), BTreeMap::new())
        .unwrap();
...
fn test_search_notes_filters_by_target_type() {
    ...
    ws.update_note(&task.id, "login task".into(), BTreeMap::new())
        .unwrap();
    let other = create_note_with_type(&mut ws, "OtherNote");
    ws.update_note(&other.id, "login other".into(), BTreeMap::new())
        .unwrap();
...
fn test_search_notes_matches_text_fields() {
    ...
    fields.insert(
        "email".into(),
        FieldValue::Email("alice@example.com".into()),
    );
...
fn operations_log_always_records_create_note() {
    ...
    let create_ops: Vec<_> = ops
        .iter()
        .filter(|o| o.operation_type == "CreateNote")
        .collect();
    assert!(
        !create_ops.is_empty(),
        "CreateNote must be recorded in always-on log"
    );
}
...
fn test_workspace_has_attachment_key_when_encrypted() {
    ...
    .unwrap();
    assert!(
        ws.attachment_key().is_some(),
        "Encrypted workspace must have attachment_key"
    );
}
...
fn test_workspace_has_no_attachment_key_when_unencrypted() {
    ...
    .unwrap();
    assert!(
        ws.attachment_key().is_none(),
        "Unencrypted workspace must have no attachment_key"
    );
}
...
fn test_workspace_attachment_key_stable_across_open() {
    ...
    assert_eq!(
        key1, *key2,
        "Key must be derived deterministically from password + workspace_id"
    );
}
...
fn test_attach_file_stores_metadata_and_file() {
    ...
    let meta = ws
        .attach_file(root_id, "test.txt", Some("text/plain"), data, None)
        .unwrap();
...
fn test_get_attachment_bytes_decrypts_correctly() {
    ...
    let meta = ws
        .attach_file(&root_id, "doc.txt", None, data, None)
        .unwrap();
...
fn test_get_attachments_returns_metadata_list() {
    ...
    ws.attach_file(&root_id, "a.pdf", None, b"data a", None)
        .unwrap();
    ws.attach_file(&root_id, "b.pdf", None, b"data b", None)
        .unwrap();
...
fn test_delete_attachment_soft_deletes() {
    ...
    let meta = ws
        .attach_file(&root_id, "bye.txt", None, b"temp", None)
        .unwrap();
    let enc_path = dir
        .path()
        .join("attachments")
    ...
    assert!(
        trash_path.exists(),
        ".enc.trash must exist after soft-delete"
    );
    assert!(
        ws.get_attachments(&root_id).unwrap().is_empty(),
        "DB row must be gone"
    );
    assert!(
        !ws.can_undo(),
        "attachment deletion without signing key must NOT push to main undo stack"
    );
}
...
fn test_attach_file_with_signing_key_logs_op_and_pushes_undo() {
    ...
    let meta = ws
        .attach_file(&root_id, "test.txt", Some("text/plain"), data, Some(&key))
        .unwrap();
    ...
    assert!(
        has_add_op,
        "AddAttachment op should be logged when signing key is provided"
    );
    ...
    assert!(
        ws.can_undo(),
        "attach_file with signing key should push undo"
    );
}
...
fn test_delete_attachment_with_signing_key_logs_op_and_pushes_undo() {
    ...
    assert!(
        !ws.can_undo(),
        "attach_file without signing key must not push undo"
    );
    ...
    assert!(
        has_remove_op,
        "RemoveAttachment op should be logged when signing key is provided"
    );

    // Verify: undo is available.
    assert!(
        ws.can_undo(),
        "delete_attachment with signing key should push undo"
    );
}
...
fn test_attach_file_enforces_size_limit() {
    ...
    assert!(matches!(
        result,
        Err(KrillnotesError::AttachmentTooLarge { .. })
    ));
}
...
fn test_update_note_cleans_up_replaced_file_field() {
    ...
    let meta1 = ws
        .attach_file(&root_id, "a.png", Some("image/png"), b"fake_bytes_1", None)
        .unwrap();
    ...
    fields.insert(
        "photo".to_string(),
        FieldValue::File(Some(meta1.id.clone())),
    );
    ws.update_note(&root_id, "Test".to_string(), fields)
        .unwrap();

    // Attach a second file and replace the field value with it.
    let meta2 = ws
        .attach_file(&root_id, "b.png", Some("image/png"), b"fake_bytes_2", None)
        .unwrap();
    let mut fields2 = ws.get_note(&root_id).unwrap().fields.clone();
    fields2.insert(
        "photo".to_string(),
        FieldValue::File(Some(meta2.id.clone())),
    );
    ws.update_note(&root_id, "Test".to_string(), fields2)
        .unwrap();
    ...
    assert!(
        result.is_err(),
        "old attachment should have been deleted when field value was replaced"
    );

    // The second attachment must still be readable.
    assert!(
        ws.get_attachment_bytes(&meta2.id).is_ok(),
        "new attachment should still exist"
    );
}
...
fn test_update_note_cleans_up_cleared_file_field() {
    ...
    let meta = ws
        .attach_file(&root_id, "x.png", Some("image/png"), b"fake", None)
        .unwrap();
    let mut fields = ws.get_note(&root_id).unwrap().fields.clone();
    ...
    assert!(
        result.is_err(),
        "attachment should have been deleted when field was cleared"
    );
}
...
fn test_undo_create_note() {
    ...
    assert!(
        ws.get_note(&child_id).is_err(),
        "note must be gone after undo"
    );
}
...
fn test_undo_update_note_restores_old_title() {
    ...
    ws.update_note(&root_id, "New Title".into(), BTreeMap::new())
        .unwrap();
    assert!(ws.can_undo());
...
fn test_undo_delete_script_restores_it() {
    ...
    assert!(
        !ws.can_undo(),
        "note undo stack must be unaffected by script ops"
    );
    assert!(matches!(
        ws.script_undo_stack[0].inverse,
...
fn test_tree_action_collapses_to_one_undo_entry() {
    ...
    assert_eq!(
        ws.undo_stack.len(),
        1,
        "three creates must collapse to one undo step"
    );
}
...
fn test_undo_redo_update_script_full_cycle() {
    ...
    assert_eq!(
        after_undo.source_code, src_v1,
        "script_undo should restore v1"
    );
    ...
    assert_eq!(
        after_redo.source_code, src_v2,
        "script_redo should re-apply v2"
    );
    ...
    assert_eq!(
        final_state.source_code, src_v1,
        "second script_undo should restore v1 again"
    );
}
...
fn test_undo_redo_create_script_full_cycle() {
    ...
    ws.script_undo().unwrap();
    assert!(
        ws.get_user_script(&script.id).is_err(),
        "script deleted by script_undo"
    );
    ...
    let after_redo = ws.get_user_script(&script.id).unwrap();
    assert_eq!(
        after_redo.source_code, src,
        "script_redo must restore real source, not empty"
    );
}
...
fn test_info_json_written_on_create() {
    ...
    assert!(
        ...
        "info.json must exist after create"
    );
}
...
fn test_info_json_written_on_open() {
    ...
    assert!(
        dir.path().join("info.json").exists(),
        "info.json must be rewritten on open"
    );
}
...
fn test_hlc_counter_increments_for_rapid_ops() {
    ...
    assert_eq!(
        unique_ids.len(),
        ops.len(),
        "all operation_ids must be unique"
    );
}
...
fn test_set_tags_op_logged() {
    ...
    ws.update_note_tags(&root.id, vec!["crdt".into(), "hlc".into()])
        .unwrap();
    ...
    assert!(
        has_set_tags,
        "SetTags operation must appear in the log after update_note_tags"
    );
}
...
fn test_update_note_op_logged() {
    ...
    ws.update_note_title(&root.id, "HLC Title".to_string())
        .unwrap();
...
fn test_on_save_gated_model() {
    ...
    let updated = ws
        .update_note(&note_id, "ignored".to_string(), fields)
        .unwrap();
...
schema("PipeItem", #{ version: 1,
    ...
    let root = ws.list_all_notes().unwrap()[0].clone();
    let note_id = ws
        .create_note(&root.id, AddPosition::AsChild, "PipeItem")
        .unwrap();
    let mut fields = BTreeMap::new();
    fields.insert("value".to_string(), FieldValue::Text("hello".into()));
    let result = ws
        .save_note_with_pipeline(&note_id, "My Item".to_string(), fields)
        .unwrap();
    assert!(
        matches!(result, SaveResult::Ok(_)),
        "expected Ok, got: {:?}",
        result
    );
}
...
schema("RatedItem", #{ version: 1,
    ...
    let root = ws.list_all_notes().unwrap()[0].clone();
    let note_id = ws
        .create_note(&root.id, AddPosition::AsChild, "RatedItem")
        .unwrap();
    let mut fields = BTreeMap::new();
    fields.insert("score".to_string(), FieldValue::Number(-1.0));
    let result = ws
        .save_note_with_pipeline(&note_id, "Item".to_string(), fields)
        .unwrap();
    match result {
...
schema("RejectItem", #{ version: 1,
    ...
    let root = ws.list_all_notes().unwrap()[0].clone();
    let note_id = ws
        .create_note(&root.id, AddPosition::AsChild, "RejectItem")
        .unwrap();
    let result = ws
        .save_note_with_pipeline(&note_id, "Item".to_string(), BTreeMap::new())
        .unwrap();
    match result {
        SaveResult::ValidationErrors { note_errors, .. } => {
            assert!(
                !note_errors.is_empty(),
                "expected note_errors from reject()"
            );
        }
        other => panic!("expected ValidationErrors, got: {:?}", other),
    }
...
schema("Grouped", #{ version: 1,
    ...
    let root = ws.list_all_notes().unwrap()[0].clone();
    let note_id = ws
        .create_note(&root.id, AddPosition::AsChild, "Grouped")
        .unwrap();
    ...
    let result = ws
        .save_note_with_pipeline(&note_id, "Note".to_string(), fields)
        .unwrap();
    assert!(
        matches!(result, SaveResult::Ok(_)),
        "test 1 failed: {:?}",
    ...
    match result {
        SaveResult::ValidationErrors { field_errors, .. } => {
            assert!(
                field_errors.contains_key("b_value"),
                "expected b_value error, got: {:?}",
                field_errors
            );
        }
    ...
    match result {
        SaveResult::ValidationErrors { note_errors, .. } => {
            assert!(
                !note_errors.is_empty(),
                "test 3 failed: expected note_errors"
            );
        }
...
schema("RequiredItem", #{ version: 1,
    ...
    let root = ws.list_all_notes().unwrap()[0].clone();
    let note_id = ws
        .create_note(&root.id, AddPosition::AsChild, "RequiredItem")
        .unwrap();
    ...
    match result {
        SaveResult::ValidationErrors { field_errors, .. } => {
            assert!(
                field_errors.contains_key("sku"),
                "expected sku required error, got: {:?}",
                field_errors
            );
        }
        other => panic!(
            "expected ValidationErrors for missing required field, got: {:?}",
            other
        ),
    }
}
...
schema("MigType", #{
    ...
    let root_id = ws.list_all_notes().unwrap()[0].id.clone();
    let note_id = ws
        .create_note(&root_id, AddPosition::AsChild, "MigType")
        .unwrap();
    ...
    fields.insert(
        "phone".to_string(),
        FieldValue::Text("555-1234".to_string()),
    );
    ws.update_note(&note_id, "Contact".to_string(), fields)
        .unwrap();
...
schema("MigType", #{
    ...
    assert_eq!(
        results.len(),
        1,
        "expected 1 migration result, got: {:?}",
        results
    );
    ...
    assert_eq!(
        note.fields.get("mobile"),
        Some(&FieldValue::Text("555-1234".to_string()))
    );
    assert!(
        !note.fields.contains_key("phone"),
...
schema("UpToDateType", #{
    ...
    let root_id = ws.list_all_notes().unwrap()[0].id.clone();
    ws.create_note(&root_id, AddPosition::AsChild, "UpToDateType")
        .unwrap();
...
schema("ChainType", #{
    ...
    let root_id = ws.list_all_notes().unwrap()[0].id.clone();
    let note_id = ws
        .create_note(&root_id, AddPosition::AsChild, "ChainType")
        .unwrap();
    let mut fields = BTreeMap::new();
...
schema("ChainType", #{
    ...
    assert_eq!(
        note.fields.get("val"),
        Some(&FieldValue::Text("original_v2_v3".to_string()))
    );
}
...
schema("SameVerType", #{
    ...
    assert_eq!(
        schema.fields.len(),
        2,
        "expected 2 fields after same-version re-registration"
    );
}
...
fn test_to_snapshot_json_includes_attachments() {
    ...
    let mut ws = Workspace::create(&db_path, "", "test-id", key, test_gate(), None).unwrap();
    let root_id = ws.list_all_notes().unwrap()[0].id.clone();
    ws.attach_file(&root_id, "test.txt", None, b"hello bytes", None)
        .unwrap();
    let json = ws.to_snapshot_json().unwrap();
    let snap: WorkspaceSnapshot = serde_json::from_slice(&json).unwrap();
...
fn test_create_with_id_preserves_uuid() {
    ...
    let ws = Workspace::create_with_id(&db_path, "", "test-id", key, custom_id, test_gate(), None)
        .unwrap();
...
fn test_create_empty_with_id_no_root_note() {
    ...
    let ws =
        Workspace::create_empty_with_id(&db_path, "", "test-id", key, custom_id, test_gate(), None)
            .unwrap();
    assert_eq!(ws.workspace_id(), custom_id);
...
fn test_create_empty_with_id_skips_starter_scripts() {
    ...
    assert_eq!(
        count, 0,
        "create_empty_with_id should not seed starter scripts"
    );
}
...
fn test_import_snapshot_json_round_trip() {
    ...
    src.create_note(&root.id, AddPosition::AsChild, "TextNote")
        .unwrap();
    src.create_note(&root.id, AddPosition::AsChild, "TextNote")
        .unwrap();
    ...
    dst.storage
        .connection_mut()
        .execute("DELETE FROM notes WHERE id = ?", [&dst_root.id])
        .unwrap();
...
fn test_snapshot_includes_permission_ops() {
    ...
    assert_eq!(
        count, 1,
        "permission op must be in destination operations log"
    );

    // Verify it was tagged as coming from a snapshot.
    let received_from: String = dst
        .storage
        .connection()
        .query_row(
...
fn test_is_leaf_blocks_create_child() {
    ...
    assert!(
        result.is_err(),
        "expected error when adding child to leaf note"
    );
    let err = result.unwrap_err().to_string();
    assert!(
        err.to_lowercase().contains("leaf"),
        "expected 'leaf' in error: {err}"
    );
}
...
fn test_is_leaf_blocks_move_note() {
    ...
    assert!(
        result.is_err(),
        "expected error when moving note under leaf"
    );
    let err = result.unwrap_err().to_string();
    assert!(
        err.to_lowercase().contains("leaf"),
...
fn test_is_leaf_blocks_deep_copy() {
    ...
    assert!(
        result.is_err(),
        "expected error when deep-copying note under leaf"
    );
    let err = result.unwrap_err().to_string();
    assert!(
        err.to_lowercase().contains("leaf"),
...
fn test_list_peers_info_known_contact() {
    ...
    let cm = ContactManager::for_identity(cm_dir.path().to_path_buf(), key).unwrap();
    let contact = cm
        .create_contact("Bob", pubkey, TrustLevel::CodeVerified)
        .unwrap();
    ...
    assert_eq!(
        peers[0].contact_id.as_deref(),
        Some(expected_contact_id.as_str())
    );
}
...
fn test_operations_since_watermark() {
    ...
    let since_ops = ws
        .operations_since(Some(&first_op_id), "nonexistent-device")
        .unwrap();
...
fn test_operations_since_excludes_device() {
    ...
    let current_device_id: String = ws
        .connection()
        .query_row(
...
fn test_apply_incoming_hlc_advances() {
    ...
    let stored_wall_ms: i64 = ws
        ...
        .unwrap();
    assert_eq!(
        stored_wall_ms as u64, far_future_ms,
...
fn test_create_user_script_rejected_for_non_owner() {
    ...
    let workspace =
        Workspace::create(temp.path(), "", "identity-a", key_a, test_gate(), None).unwrap();
    drop(workspace);

    let key_b = ed25519_dalek::SigningKey::from_bytes(&[2u8; 32]);
    let mut workspace =
        Workspace::open(temp.path(), "", "identity-b", key_b, test_gate(), None).unwrap();
...
fn test_update_user_script_rejected_for_non_owner() {
    ...
    let mut workspace =
        Workspace::open(temp.path(), "", "identity-b", key_b, test_gate(), None).unwrap();
    let result = workspace.update_user_script(
        &script_id,
...
fn test_owner_pubkey_matches_creator() {
    ...
    let key = ed25519_dalek::SigningKey::from_bytes(&[1u8; 32]);
    let workspace =
        Workspace::create(temp.path(), "", "test-id", key.clone(), test_gate(), None).unwrap();
...
fn test_open_legacy_workspace_without_owner_pubkey_assigns_opener() {
    ...
    workspace
        .connection()
        .execute("DELETE FROM workspace_meta WHERE key = 'owner_pubkey'", [])
        .unwrap();
    drop(workspace);
...
fn test_list_peers_by_channel() {
    ...
    ws.update_peer_channel(
        "identity:relay-identity",
        "relay",
        r#"{"relay_url":"https://example.com"}"#,
    )
    .unwrap();
...
fn test_undo_add_attachment() {
    ...
    assert!(
        ws.can_undo(),
        "should be able to undo after attach_file with signing key"
    );
    ...
    assert!(
        !enc_path.exists(),
        ".enc must not exist after undo of attach_file"
    );
    assert!(
        trash_path.exists(),
        ".enc.trash must exist after undo of attach_file"
    );
    assert!(
        !ws.can_undo(),
    ...
    ws.redo().unwrap();
    assert_eq!(
        ws.get_attachments(&root_id).unwrap().len(),
        1,
        "attachment must be back after redoing attach_file"
    );
    assert!(
        enc_path.exists(),
        ".enc must exist after redo of attach_file"
    );
    assert!(
        !trash_path.exists(),
        ".enc.trash must not exist after redo of attach_file"
    );
...
fn test_undo_delete_attachment() {
    ...
    let meta = ws
        .attach_file(&root_id, "del_undo.txt", Some("text/plain"), data, None)
        .unwrap();
    assert!(
        !ws.can_undo(),
        "attach_file without signing key must not push undo"
    ...
    assert!(
        !enc_path.exists(),
        ".enc must be gone after delete_attachment"
    );
    assert!(
        trash_path.exists(),
        ".enc.trash must exist after delete_attachment"
    );
    assert!(
        ws.get_attachments(&root_id).unwrap().is_empty(),
    ...
    assert!(
        ws.can_undo(),
        "should be able to undo after delete_attachment with signing key"
    );
    ...
    assert!(
        enc_path.exists(),
        ".enc must exist after undo of delete_attachment"
    );
    assert!(
        !trash_path.exists(),
        ".enc.trash must not exist after undo of delete_attachment"
    );
    assert!(
        !ws.can_undo(),
        "undo stack should be empty after single undo"
    ...
    assert!(
        !enc_path.exists(),
        ".enc must not exist after redo of delete_attachment"
    );
    assert!(
        trash_path.exists(),
        ".enc.trash must exist after redo of delete_attachment"
    );
    assert!(ws.can_undo(), "undo stack should have an entry after redo");
    assert!(!ws.can_redo(), "redo stack should be empty after redo");
}
...
fn test_register_device_fields_are_correct() {
    ...
    let data: serde_json::Value =
        serde_json::from_str(&data_json).expect("operation_data must be valid JSON");
    ...
    let device_uuid = data["device_uuid"]
        .as_str()
        .expect("device_uuid must be present");
    assert_eq!(
        device_uuid, "test-identity",
        "device_uuid should match the device_id part"
    );

    // device_name must be non-empty.
    let device_name = data["device_name"]
        .as_str()
        .expect("device_name must be present");
    assert!(!device_name.is_empty(), "device_name must be non-empty");

    let pubkey = data["identity_public_key"]
        .as_str()
        .expect("identity_public_key must be present");
    assert!(
        !pubkey.is_empty(),
        "identity_public_key must be non-empty after signing"
    );
}
...
fn test_set_note_checked() {
    ...
    assert!(
        after_undo.is_checked,
        "undo should restore checked state to true"
    );
}
...
fn test_c5_delete_note_logs_operation_atomically() {
    ...
    assert!(
        !ops.is_empty(),
        "DeleteNote operation must be logged after recursive delete"
    );
}
...
fn test_m4_undo_delete_schema_script_restores_category() {
    ...
    assert_eq!(
        restored.category, "schema",
        "restored script must retain original category, not default to 'library'"
    );
}
...
fn test_m5_purge_retains_more_than_100_ops() {
    ...
    for i in 0..120 {
        ws.update_note_title(&root.id, format!("Title {i}"))
            .unwrap();
    }
...
fn test_apply_incoming_create_note_stores_seconds_timestamp() {
    ...
    assert_eq!(
        row.0, expected_secs,
        "created_at should be seconds ({}), not milliseconds ({})",
        expected_secs, known_wall_ms
    );
    assert_eq!(
        row.1, expected_secs,
        "modified_at should be seconds ({}), not milliseconds ({})",
        expected_secs, known_wall_ms
    );
    assert!(
        row.0 < 10_000_000_000,
        "created_at looks like milliseconds: {}",
        row.0
    );
}
