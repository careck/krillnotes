...
fn test_export_workspace_creates_valid_zip() {
    ...
    assert!(
        manifest.scripts.len() >= 2,
        "Should have starter scripts plus user script"
    );
    let widget_entry = manifest
        .scripts
        .iter()
        .find(|s| s.filename == "custom-widget.rhai");
    assert!(
        widget_entry.is_some(),
        "Should contain custom-widget.rhai in manifest"
    );
...
fn test_peek_import_reads_metadata() {
    ...
    assert_eq!(result.note_count, 1); // root note
                                      // Starter scripts + the user-created Widget script
    assert!(
        result.script_count >= 2,
        "Should have starters + user script, got {}",
        result.script_count
    );
}
...
fn test_round_trip_export_import() {
    ...
    ws.update_note_title(&root.id, "Root Note".to_string())
        .unwrap();

    let child_id = ws
        .create_note(&root.id, AddPosition::AsChild, "TextNote")
        .unwrap();
    ws.update_note_title(&child_id, "Child Note".to_string())
        .unwrap();
    ...
    let imported_ws = Workspace::open(
        temp_dst.path(),
        "",
        "test-identity",
        ed25519_dalek::SigningKey::from_bytes(&[1u8; 32]),
...
fn test_round_trip_preserves_script_category() {
    ...
    ws.create_user_script_with_category(schema_src, "schema")
        .unwrap();
    ws.create_user_script_with_category(lib_src, "library")
        .unwrap();
    ...
    assert_eq!(
        schema_script.category, "schema",
        "schema script category must survive export/import round-trip"
    );
    assert_eq!(
        lib_script.category, "library",
        "library script category must survive export/import round-trip"
    );
}
...
fn test_export_with_password_creates_encrypted_zip() {
    ...
    assert!(
        notes_file.encrypted(),
        "notes.json must be encrypted when password is provided"
    );
}
...
fn test_export_without_password_creates_plain_zip() {
    ...
    assert!(
        !notes_file.encrypted(),
        "notes.json must be plain when no password given"
    );
}
...
fn test_encrypted_round_trip_import() {
    ...
    .unwrap();
    let root = ws.list_all_notes().unwrap()[0].clone();
    ws.update_note_title(&root.id, "Encrypted Root".to_string())
        .unwrap();
...
fn test_import_notes_without_tags_field() {
    ...
    assert!(
        notes[0].tags.is_empty(),
        "imported note from old archive should have no tags"
    );
}
...
fn test_export_includes_attachments() {
    ...
    let mut archive = zip::ZipArchive::new(Cursor::new(&buf)).unwrap();
    assert!(
        archive.by_name("attachments.json").is_ok(),
...
fn test_import_restores_attachments() {
    ...
    ws.attach_file(&root_id, "data.txt", None, b"attachment content", None)
        .unwrap();
...
fn test_peek_import_returns_none_metadata_for_old_archives() {
    let mut buf = Vec::new();
    {
        ...
        zip.start_file("notes.json", options).unwrap();
        zip.write_all(serde_json::to_string(&notes).unwrap().as_bytes())
            .unwrap();
        zip.finish().unwrap();
    ...
    assert!(
        ...
        "old archives without workspace.json should return None metadata"
    );
}
