    ...
    <div className="fixed inset-0 z-70 flex items-center justify-center bg-black/50">
      <div className="bg-background border border-border rounded-lg shadow-xl w-[400px] max-h-[480px] flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 border-b border-border">
          <h2 className="text-base font-semibold">{t('peers.addFromContactsTitle')}</h2>
          <button onClick={onClose} className="text-muted-foreground hover:text-foreground">✕</button>
        </div>

        <div className="flex-1 overflow-y-auto p-4 space-y-1.5">
          {loading && (
            <p className="text-sm text-muted-foreground text-center py-6">{t('peers.loadingContacts')}</p>
          )}
          {!loading && contacts.length === 0 && (
            <p className="text-sm text-muted-foreground text-center py-6">
              {t('peers.allContactsArePeers')}
            </p>
          )}
          {error && (
        ...
        <div className="flex items-center justify-end gap-2 p-4 border-t border-border">
          ...
          >
            {saving ? t('peers.adding') : t('peers.addAsPeer')}
          </button>
        </div>
      </div>
    </div>
